...
`analyse_sample_invoice.py` dumps a raw `AnalyzeResult` for one sample. `map_extraction_samples.py` fills the Pydantic invoice and receipt models and compares them to `samples/manifest.json`. Cached files under `output/` are reused so repeat runs do not spend extra Azure pages. `create_openai_response.py` sends a prompt through the Azure OpenAI Responses API. `classify_sample_document.py` sends the original PDF or image through the Pydantic AI classifier and prints a typed invoice/receipt label. `process_sample_document.py` chains classification, Document Intelligence extraction, and VAT/totals checks; INFO logs show each pipeline step.
