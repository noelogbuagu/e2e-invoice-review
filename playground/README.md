...
**Primary workflow:** open a script in the interactive window, edit the constants at the top (`DOCUMENT_PATH`, `PROMPT`, and so on), then run `main()`. `bootstrap.py` patches asyncio and wires up backend imports so sync APIs work from a notebook kernel.
